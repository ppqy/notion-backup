...
产品界面以中文为主。运行时是一个单体 Fastify 服务：它负责托管 React 面板、提供 API、运行备份 Worker，并把备份产物写入本地磁盘或 Docker 数据卷。
...
Compose 配置会把应用数据保存到 `notion-backup-data` 数据卷，并挂载到容器内的 `/data`。
...
Docker 会把这些文件存放在 `/data` 数据卷下。本地开发默认使用相同结构的 `./data` 目录。
...
- 请保护 Docker 数据卷或 `./data` 目录。它包含 SQLite 数据库、加密 token、运行元数据和备份产物。
